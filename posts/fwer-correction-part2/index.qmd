---
title: "Permutation-Based FWER Correction, Part 2: Null Distributions and P-Values"
...
In the first part of this series, I introduced the basics of hypothesis testing: null vs. alternative hypotheses, test statistics, Type I errors, significance levels, and a preliminary intuition for p-values. I also worked through a simple coin-flip example. In this second post, I will build on that foundation to discuss **null distributions** and the formal definition of a p-value. I explore how null distributions can be derived both analytically and through simulation, clarify the interpretation of p-values (and what they do and do not tell us), examine the difference between the distribution of a test statistic under $H_0$ and under $H_1$, and explain how **critical values** relate to p-values. As before, we will include R code examples – for instance, to empirically generate a null distribution via simulation and to compute p-values using both theoretical formulas and simulation-based methods. These concepts are crucial stepping stones toward understanding permutation-based tests and FWER correction.
...
Understanding null distributions and p-values is a critical step in that journey. We now have a solid grasp of what p-values represent, how to obtain them, and how to interpret them in the context of hypothesis tests. In the next part of this series, I will delve into multiple testing and error rates (including FWER and the false discovery rate) and introduce permutation-based approaches to control these error rates. We’ll show how permutation tests can be applied in a neuroscience context (e.g. analyzing EEG or fMRI data) to make robust inferences while accounting for the multiplicity of comparisons.
