...
```{r}
permute_k_test <- function(data, K.max=10, B=100, alpha=0.05){
  n <- nrow(data)
  # Ensure data is scaled or appropriate for Euclidean distance
  # (I might scale outside before calling this if needed)
  optK <- 1
  prevW <- sum(scale(data, center=TRUE, scale=FALSE)^2)  # WCSS for K=1 (total SS)
  for(K in 2:K.max){
    # Compute real WCSS for K clusters
    kmK <- kmeans(data, centers=K, nstart=10)
    W_K <- kmK$tot.withinss
    # Using prevW for W(K-1) from last iteration (or I could recompute kmeans for K-1 if not stored)
    ...
    for(b in 1:B){
      ...
      for(j in 1:ncol(data_perm)){
        data_perm[,j] <- sample(data_perm[,j])
      }
      # cluster permuted data
      kmK_perm <- kmeans(data_perm, centers=K, nstart=1, iter.max=50)
      kmKm1_perm <- kmeans(data_perm, centers=K-1, nstart=1, iter.max=50)
      W_K_perm <- kmK_perm$tot.withinss
      W_Km1_perm <- kmKm1_perm$tot.withinss
      delta_perm[b] <- W_Km1_perm - W_K_perm
    }
    # p-value: fraction of permuted deltas >= real delta
    p_val <- mean(delta_perm >= delta_real)
    cat("K =", K, ": WCSS =", W_K, ", prev WCSS =", prevW,
        ", Δ =", delta_real, ", p =", p_val, "\n")
    if(p_val < alpha){
      optK <- K
      prevW <- W_K  # update for next iteration
      # continue loop
    } else {
      # not significant improvement, stop
      break
    }
  }
  return(optK)
...
# Create each cluster with explicit X and Y components
cluster1 <- data.frame(
  X1 = rnorm(n, mean = 0, sd = sd),
  X2 = rnorm(n, mean = 0, sd = sd),
  label = 1
)
cluster2 <- data.frame(
  X1 = rnorm(n, mean = 5, sd = sd),
  X2 = rnorm(n, mean = 0, sd = sd),
  label = 2
)
cluster3 <- data.frame(
  X1 = rnorm(n, mean = 0, sd = sd),
  X2 = rnorm(n, mean = 5, sd = sd),
  label = 3
)
cluster4 <- data.frame(
  X1 = rnorm(n, mean = 5, sd = sd),
...
plot(
  data.sim$X1, data.sim$X2, col = data.sim$label,
  pch = 16, cex = 0.6,
  main = "Simulated Data: 4 Clusters at Corners",
  xlab = "X1", ylab = "X2"
)
#legend("topright", legend = paste("Cluster", 1:4), col = 1:4, pch = 16)
...
```{r}
result <- permute_k_test(data.sim, K.max=8, B=50, alpha=0.01)
cat("Chosen K:", result, "\n")
```
...
K = 3 : ... p = 0.000
K = 4 : ... p = 0.000
K = 5 : ... p = 0.15   <-- not significant at 0.01
Chosen K: 4
`
...
res <- permute_k_test(as.data.frame(data.unif), K.max=5, B=50, alpha=0.05)
...
```{r}
res_iris <- permute_k_test(as.data.frame(iris[, -5]), K.max=10, B=100, alpha=0.05)
```
...
Interestingly, you could also incorporate other metrics into a permutation test – for example, test if the silhouette score at a given K is significantly higher than under random assignments. The multi-aspect permutation test by Ceccato et al. (2025) combines multiple such metrics in a nonparametric combination. My focus on WCSS is narrower but straightforward.
