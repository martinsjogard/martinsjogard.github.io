...
n <- 200
data.sim <- rbind(
  matrix(rnorm(n*2, mean=0, sd=0.3), ncol=2),
  matrix(rnorm(n*2, mean=5, sd=0.3), ncol=2),
  matrix(rnorm(n*2, mean=c(0,5), sd=0.3), ncol=2),
  matrix(rnorm(n*2, mean=c(5,5), sd=0.3), ncol=2)
)
data.sim <- as.data.frame(data.sim)
colnames(data.sim) <- c("X1","X2")
plot(data.sim, col="grey", pch=16, cex=0.5)
```
