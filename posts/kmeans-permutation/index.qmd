...
```{r}
library(ggpl)
permute_k_test <- function(data, K.max = 10, B = 200, alpha = 0.05, verbose = TRUE) {
  n <- nrow(data)
  p <- ncol(data)
  
  W_real <- numeric(K.max)       # WCSS for each K
  delta_real <- numeric(K.max)   # Δ WCSS = W(K-1) - W(K)
  delta_perm_all <- list()       # store each set of permuted Δs
  
  W_real[1] <- sum(scale(data, center = TRUE, scale = FALSE)^2)  # total SS for K=1
  optK <- 1
  
  for (K in 2:K.max) {
    kmK <- kmeans(data, centers = K, nstart = 10)
    W_real[K] <- kmK$tot.withinss
    delta_real[K] <- W_real[K - 1] - W_real[K]
    ...
    for (b in 1:B) {
      ...
      kmK_perm   <- kmeans(data_perm, centers = K, nstart = 10, iter.max = 50)
      kmKm1_perm <- kmeans(data_perm, centers = K - 1, nstart = 10, iter.max = 50)
      
      delta_perm[b] <- kmKm1_perm$tot.withinss - kmK_perm$tot.withinss
    }
    delta_perm_all[[K]] <- delta_perm
    p_val <- mean(delta_perm >= delta_real[K])
    
    if (verbose) {
      cat(sprintf("K = %d | Δ_real = %.2f | p = %.4f\n", K, delta_real[K], p_val))
    }
    
    if (p_val < alpha) {
      optK <- K
    } else {
      if (verbose) cat("Stopping at K =", optK, "\n")
      break
    }
  }
  
  return(list(optK = optK,
              W_real = W_real[1:K.max],
              delta_real = delta_real[1:K.max],
              delta_perm_all = delta_perm_all))
}
```
...
```{r}
result <- permute_k_test(data.sim, K.max=8, B=1000, alpha=0.05)
```
